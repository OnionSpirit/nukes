/**
 * @file
 * @details Contains atomic_roaming_mpsc_queue_base declaration
 */
#ifndef NUKES_DYNAMIC_ROAMING_MPSC_QUEUE
#define NUKES_DYNAMIC_ROAMING_MPSC_QUEUE
...
/**
 * @brief An atomic roaming-mpsc queue
 * @details The atomic roaming-mpsc queue base class, can be used as dyn spsc queue because they have same realization.
 * <br>Difference between roaming-mpsc and usual mpsc is that @b head pointer of the roaming-mpsc is atomic
 * to keep it synchronized between caches.
 * <br>The roaming-mpsc is required when there is a single consumer at the moment of the consumption,
 * but may have changed between the consumption operation.
...
private:
    ...
    class dyn_roaming_mpsc_iter {

        roaming_mpsc_queue* _queue { nullptr };

    public:
        explicit dyn_roaming_mpsc_iter(roaming_mpsc_queue* queue)
            : _queue(queue) {}

        dyn_roaming_mpsc_iter& postfix_increment(details::misc::argument_ref_t<node_t*> ptr) {
        ...
        dyn_roaming_mpsc_iter prefix_increment(details::misc::argument_ref_t<node_t*> ptr)  {
...
#define DYNAMIC_ROAMING_MPSC_QUEUE_MEMBER(member_type)         \
    template <typename dataT>                          \
        member_type nukes::dynamic::roaming_mpsc_queue<dataT>::
...
#undef DYNAMIC_ROAMING_MPSC_QUEUE_MEMBER
#endif // NUKES_DYNAMIC_ROAMING_MPSC_QUEUE
