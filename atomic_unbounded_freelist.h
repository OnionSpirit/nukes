...
>
requires (constants::ufl_memory_offset + sizeof(meta_chunk<ChunkType, constants::bucket_meta_data>) <= BucketByteSize)
struct atomic_unbounded_freelist {

protected:

    typedef meta_chunk<ChunkType, constants::bucket_meta_data> bucket_chunk_t;

    static constexpr size_t bucket_chunk_count =
        (BucketByteSize - constants::ufl_memory_offset) / sizeof(bucket_chunk_t); ///< Wanted to fit whole bucket_node
                                                                                ///< with bucket into BucketBytesCount

    typedef atomic_bounded_freelist<bucket_chunk_t, bucket_chunk_count> bucket_t;
...
sync(ChunkType*& ptr) noexcept {
    ...
    bucket_chunk_t* mem = (bucket_chunk_t*)((uint8_t*)ptr - sizeof(meta_data<>));
